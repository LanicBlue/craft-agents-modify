 ...
 * in-process pi coding agent SDK — the exact path Project Service would take.
 *
 ...
 *   (harness branch) → PiSdkDriver → real pi SDK (local ~/.pi config)
 *   → native session file persisted → disk persistence → restart → same
 *   session file (resume by exact path).
 *
 * Gated by CRAFT_PI_SDK_E2E=1.
 * PREREQUISITE: the local `pi` (or the pi SDK's default provider) must be
 * authenticated on this machine (~/.pi provider auth). Without it the run
 * fails with the provider's auth error — see the gated test output.
 *
...
const E2E = process.env.CRAFT_PI_SDK_E2E === '1'

describe.skipIf(!E2E)('agentSessions:dispatch end-to-end over real RPC + real pi SDK', () => {
  it('dispatch → reuse → restart resume, one canonical pi session file', async () => {
    ...
    const TOKEN = 'e2e-pi-dispatch-token-with-enough-entropy'
    ensureConfigDir()
    saveConfig({ workspaces: [], activeWorkspaceId: null, activeSessionId: null })
    const wsRoot = mkdtempSync(join(tmpdir(), 'e2e-rpc-pi-ws-'))
    const workspace = addWorkspace({ name: 'E2E RPC PI WS', rootPath: wsRoot })

    registerBuiltinHarnessDrivers() // codex + claude + pi (idempotent)
    ...
    async function withRpc<T>(
      ...
      const server = new WsRpcServer({
        ...
        validateToken: async (t) => t === TOKEN, serverId: 'e2e-pi',
      })
      await server.listen()
    ...
    try {
      ...
      const marker = `PI-PONG-${Date.now().toString(36)}`
      let sessionId = ''
      let sdkSessionId = ''
      ...
      await withRpc(sm1, async (invoke) => {
        const agent = await invoke('agents:create' as never, {
          id: 'e2e-rpc-pi-agent',
          name: 'E2E RPC PI Agent',
          execution: { kind: 'external-harness', harness: 'pi', configMode: 'local-inherit' },
          systemPrompt: 'You are the e2e pi echo agent.',
        } as never)
        expect((agent as { id?: string }).id).toBe('e2e-rpc-pi-agent')

        const r1 = await invoke('agentSessions:dispatch' as never, 'E2E RPC PI WS', 'e2e-rpc-pi-agent',
        ...
        expect(sdkSessionId.length).toBeGreaterThan(0) // native pi session file persisted
        ...
        const rt = await invoke('agentSessions:getRuntime' as never, 'E2E RPC PI WS', 'e2e-rpc-pi-agent') as {
        ...
        const r2 = await invoke('agentSessions:dispatch' as never, 'E2E RPC PI WS', 'e2e-rpc-pi-agent',
          'Reply with exactly: PI-SECOND') as { sessionId: string; bindingGeneration: number }
      ...
      const binding = getBinding(wsRoot, 'e2e-rpc-pi-agent')
      ...
      await withRpc(sm2, async (invoke) => {
        const r3 = await invoke('agentSessions:dispatch' as never, 'E2E RPC PI WS', 'e2e-rpc-pi-agent',
          'Reply with exactly: PI-RESTART') as { sessionId: string; bindingGeneration: number }
        expect(r3.sessionId).toBe(sessionId)      // same canonical session
        expect(r3.bindingGeneration).toBe(1)      // binding not rotated
        await sessionPersistenceQueue.flush(sessionId)
        const stored = loadSession(wsRoot, sessionId)
        expect(stored!.sdkSessionId).toBe(sdkSessionId) // resumed the SAME pi session file — no fork
        const texts = stored!.messages.map((m) => ({ type: m.type, content: m.content ?? '' }))
        expect(texts.some((m) => m.type === 'assistant' && m.content.includes('PI-RESTART'))).toBe(true)
      ...
      expect(firstLine.agentId).toBe('e2e-rpc-pi-agent')
    } finally {
